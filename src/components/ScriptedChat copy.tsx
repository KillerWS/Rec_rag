...
//   const [isStarted, setIsStarted] = useState(false);
  ...
  const hasSentInitialQuestionRef = useRef<boolean>(false);
  const [socialPreference, setSocialPreference] = useState<string>(""); // review focus
  ...
  const conversationSteps = [
    ...
    { key: "Review Focus", question: "We’ve collected your preferences. Would you like to explore guest review insights next, or generate recommendations now?" }
  ];

  // Ensure first scripted question appears when already started, and hide free-text input during Price step
  useEffect(() => {
    if (isFinalized) return;
    if (!isStarted) return;
    const firstQuestion = conversationSteps[0]?.question;
    const hasAskedFirstQuestion = messages.some((m: any) => m.sender === "system" && m.text === firstQuestion);
    if (!hasAskedFirstQuestion && !hasSentInitialQuestionRef.current) {
      setIsChatAreaVisible(false);
      // 仅在 scripted 模式下插入欢迎与阶段说明
      if (mode === 'scripted') {
        const hasWelcome = messages.some((m: any) => m.sender === 'system' && m.text === 'Welcome to the Airbnb Recommendation System!');
        if (!hasWelcome) {
          appendMessage({ text: 'Welcome to the Airbnb Recommendation System!', sender: 'system' });
          appendMessage({ text: 'Phase 1: We will collect your preferences step by step to tailor the recommendations. You can refine them later.', sender: 'system' });
        }
      }
      appendMessage({ text: firstQuestion, sender: "system" });
      hasSentInitialQuestionRef.current = true;
    }
  }, [isStarted, isFinalized, messages, conversationSteps, mode]);

  // Keep local editable value for Review Focus in sync with selection list
  useEffect(() => {
    const socials = (selectedDimensions as Array<{ key: string; value: any }>)
      .filter((d) => d.key === 'Review Focus');
    if (socials.length > 0) {
      const last = socials[socials.length - 1];
      setSocialPreference(String(last.value || ''));
    }
  }, [selectedDimensions]);
  ...
  const handlePriceSubmit = () => {
    ...
    appendMessage({ text: "", sender: "system", type: "chart_option" });
                  ...
                  columns={[
                    ...
                    { 
                      ...
                      render: (value: string, record: any) => {
                        if (record.key === 'Review Focus') {
                          return (
                            <Input.TextArea
                              value={socialPreference}
                              placeholder="Describe what you care about in reviews (e.g., strong Wi‑Fi, quiet at night)"
                              autoSize={{ minRows: 2, maxRows: 4 }}
                              onChange={(e) => {
                                const newVal = e.target.value;
                                setSocialPreference(newVal);
                                setSelectedDimensions((prev: any[]) => {
                                  const withoutSocial = prev.filter((d: any) => d.key !== 'Review Focus');
                                  return [...withoutSocial, { key: 'Review Focus', value: newVal }];
                                });
                              }}
                            />
                          );
